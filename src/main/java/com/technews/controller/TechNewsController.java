...
public class TechNewsController {
    ...
    public String createCommentEditPage(@ModelAttribute Comment comment, HttpServletRequest request) {
        ...
        if (comment.getCommentText().equals("") || comment.getCommentText().equals(null)) {
            return "redirect:/editPostEmptyComment/" + comment.getPost().getId();
        } else {
            if (request.getSession(false) != null) {
                User sessionUser = (User) request.getSession().getAttribute("SESSION_USER");
                comment.setUser(sessionUser);
                ...
                return "redirect:/dashboard/edit/" + comment.getPost().getId();
            } else {
                return "redirect:/login";
            }
        }
    ...
    public void addVoteCommentsPage(@RequestBody Vote vote, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (request.getSession(false) != null) {
            ...
            User sessionUser = (User) request.getSession().getAttribute("SESSION_USER");
            vote.setUser(sessionUser);
            voteRepository.save(vote);

            returnPost = postRepository.getById(vote.getPost().getId());
            returnPost.setVoteCount(voteRepository.countVotesByPostId(vote.getPost().getId()));
        }
    }
}